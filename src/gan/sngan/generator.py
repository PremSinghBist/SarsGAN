...
class Generator(Model):
    ...
    def generate(self, z, labels, reuse=False):
        '''
        z : noise value that is passed
        '''
        with tf.variable_scope(self.scope_name) as scope:
            if reuse:
                scope.reuse_variables()
            out = self.network(z, labels, reuse) #labels are the y outputs of real dataset #Self: ResnetGenerator
            #actual, expected   [For generated data]
            self.validate_shape(out, self.output_shape)  #Shape is datahandler.shape | which is again Protein class shape 
