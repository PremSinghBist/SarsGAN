...
class ResnetGenerator(Generator):
    ...
    def network(self, z, labels, reuse):
        ...
        # And Apply linear transformation to Z to compress the size of tensor
        #This will be used as input to generator
        ...
        with tf.variable_scope("up", reuse=reuse):
            #applies series of convolution blocks to increse tensor size to desired output shape 
            #Given dilation = (1,1)
            for layer_id in range(number_of_layers):
                print(h.shape)
                block_name = 'up_block{}'.format(number_of_layers - (layer_id + 1))
                dilation_rate = (1,1)
                #Spectral normalization block 
                ...
                if layer_id == number_of_layers - 2:
                    #Use attention mechansim to focus on important part only of the dataset
        ...
        #Use batch normalization with hyperbolic tangent activation | scale between [-1  to 1]
